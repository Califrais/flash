...
def run():
    ...
    for j in range(len(datasets)):
        ...
        for i in range(50):
            ...
            learner = ext_EM(print_every=1, l_pen_SGL=l_pen_SGL,
                             l_pen_EN=l_pen_EN,
            ...
            T_u = np.unique(T_train[delta_train == 1])
            n_long_features = len(time_dep_feat)
            asso_feat_test_sig, _ = feat_representation_extraction_sig(Y_flash_test, T_u)
            asso_feat_train_sig, _ = feat_representation_extraction_sig(Y_flash_train, T_u)
            # Training with best cross-validation params
            l_pen_EN, l_pen_SGL = flash_pens[0], flash_pens[1]
            learner = ext_EM(print_every=1, l_pen_SGL=l_pen_SGL, l_pen_EN=l_pen_EN,
                             fc_parameters=final_selected_fc_parameters,
                             verbose=False, sig_ext=True)
            start = time()
            learner.fit(X_flash_train, Y_flash_train, T_train, delta_train,
                        asso_feat_train_sig, K=2)
            Flash_sig_c_index_train = learner.score(X_flash_train, Y_flash_train,
                                                T_train, delta_train,
                                                asso_feat_train_sig)
            Flash_sig_c_index = learner.score(X_flash_test, Y_flash_test, T_test,
                                          delta_test, asso_feat_test_sig)
            if Flash_sig_c_index_train < .5:
                Flash_sig_c_index_train = 1 - Flash_c_index_train
                Flash_sig_c_index = 1 - Flash_c_index
            Flash_sig_exe_time = time() - start
            ...
            tmp_df = pd.DataFrame(
                [["FLASH_sig", dataset, Flash_sig_c_index, Flash_sig_exe_time],
    ...
    if not os.path.exists("results"):
        os.mkdir("results")
    plt.savefig('results/flash_competing_sig.pdf', bbox_inches='tight')
    plt.show()
