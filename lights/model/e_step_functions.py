...
def construct_MC_samples(theta, N):
...
def f_data_given_latent(X, extracted_features, T, delta, S, theta, asso_functions, n_long_features, fixed_effect_time_order):
...
def _g1(X, T, S, theta, asso_functions, n_long_features, fixed_effect_time_order):
    """Computes g1
    ...
    g2 = _g2(T_u, S, theta, n_time_indep_features, asso_functions, n_long_features, fixed_effect_time_order)
...
def _g2(T_u, S, theta, n_time_indep_features, asso_functions, n_long_features, fixed_effect_time_order):
    """Computes g2
    ...
    p = n_time_indep_features
    gamma_0, gamma_1 = theta["gamma_0"], theta["gamma_1"]
    asso_func = get_asso_func(T_u, S, theta, asso_functions, n_long_features, fixed_effect_time_order)
    J = T_u.shape[0]
...
def _g5(T, S, theta, asso_functions, n_long_features, fixed_effect_time_order):
    """Computes g5
    ...
    """
    g5 = get_asso_func(T, S, theta, asso_functions, n_long_features, fixed_effect_time_order, derivative=True)
    return g5

def _g6(X, T, S, theta, asso_functions, n_long_features, fixed_effect_time_order):
    """Computes g6
    ...
    T_u = np.unique(T)
    n_samples = T.shape[0]
    g5 = _g5(T_u, S, theta, asso_functions, n_long_features, fixed_effect_time_order)
    g5 = np.broadcast_to(g5, (n_samples,) + g5.shape)
    g1 = _g1(X, T, S, theta, asso_functions, n_long_features, fixed_effect_time_order)
    g6 = (g1.T * g5.T).T
    return g6

def _g8(extracted_features, S, theta, n_long_features, n_samples):
    """Computes g8
