...
class SimuJointLongitudinalSurvival(Simulation):
    ...
    def simulate(self):
        ...
        # Test
        iota_0 = np.zeros(n_samples)
        iota_1 = np.zeros(n_samples)
        iota_0[G == 0] = iota_01
        iota_0[G == 1] = iota_11
        iota_1[G == 0] = iota_02
        iota_1[G == 1] = iota_12
        ...
        T = np.delete(T_star, empty_long_idx)

        iota_0 = np.delete(iota_0, empty_long_idx)
        iota_1 = np.delete(iota_1, empty_long_idx)
        T_u = np.unique(T)
        ind_1 = T.reshape(-1, 1) == T_u
        ind_2 = T.reshape(-1, 1) >= T_u

        g1 = iota_0.reshape(-1, 1) + iota_1.reshape(-1, 1).dot(T_u.reshape(1, -1))
        #print(iota_0[977], iota_0[235], iota_0[170], iota_0[601])
        #print(iota_1[977], iota_1[235], iota_1[170], iota_1[601])

        baseline_hazard = pd.Series(
            data=((((ind_1 * 1).T * delta).sum(axis=1)) /
                  (np.exp(g1).T * (ind_2 * 1).T).sum(axis=1)), index=T_u)
        self.long_features = Y

        return X, Y, T, delta, baseline_hazard
